...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $categoria = $categoriasService->findByName($name);
    if ($categoria) {
        $errores['name'] = 'Ya existe una Categoria con ese nombre.';
    }
    if (count($errores) === 0) {


        $categoriaNew= new Categoria();
        $categoriaNew->nameCategory = $name;
        $categoriaNew->isDeleted = isset($deleted);

        try {
            $categoriasService->save($categoriaNew);
            echo "<script type='text/javascript'>
                window.location.href = 'gestionCategoria.php?create=true';
                </script>";
        } catch (Exception $e) {
            echo "<script type='text/javascript'>
                alert($e)
                window.location.href = 'gestionCategoria.php?create=false';
                </script>";
        }
    }
}
